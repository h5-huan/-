...
Page({
  ...
  data: {
    ...
    advimgurl:"https://vote.south-power.cn/Public/Uploads/",
    tab2:false,
    tab4: false,
    token: '',
    session_id: '',
...
// ..................首页...................
  ...
  binddetail: function (e) {
    ...
    var token = wx.getStorageSync('token');
    var session_id = wx.getStorageSync('session_id');
    if (e.target.dataset.hasOwnProperty('uid') || e.target.dataset.hasOwnProperty('src')) {
      if (e.target.dataset.hasOwnProperty('src')){
        console.log('5555555！');
      }
      console.log('我点击的是分享！');
      return false;
    ...
    var send = { id: uid, pagetype: 'index' };
    wx.navigateTo({
  ...
  onLoad: function (options) {
    ...
    var token = wx.getStorageSync('token');
    var session_id = wx.getStorageSync('session_id');
    var _ispass = wx.getStorageSync('ispass'); 
    that.setData({
      token: token,
      session_id: session_id,
    })
    ...
    } else if (_ispass == 1 || _ispass == 3){
      that.setData({
        shscreen: true
      })
      wx.removeStorageSync('ispass');
    }
    console.log(token, session_id);
      wx.request({
    ...
    console.log(openid);
    // 邀请好友到首页
   if (options.sharedetail){
      if (!openid) {
        wx.setStorageSync('sharedetail', options.sharedetail);
        wx.reLaunch({
          url: '../welcome/index',
        })
      }else{
        var send = { id: options.sharedetail, pagetype: 'index' };
        wx.navigateTo({
          url: '../masterdetail/masterdetail?sendinfo=' + JSON.stringify(send),
        })
      }
    ...
    wx.request({
      ...
      data: {
        token: token,
        session_id: session_id,
        photo_type:'main_share'
      },
      header: { 'content-type': 'application/x-www-form-urlencoded' },
      success: function (res) {
        console.log(res);
        for (var i = 0; i < res.data.info.length;i++){
          if (res.data.info[i].photo_type =='main_share'){
